...
class Plugin:
	...
	def install_dependencies(self, installer: Installer):
		installer.log(f"Installing dependencies needed for AUR package installation: {self.DEPENDENCIES}", level=logging.INFO)
	...
	def install_aur_helper(self, installer: Installer):
		...
		installer.log(installer.arch_chroot(f"su aurinstall -c 'cd $(mktemp -d) && git clone {self.AUR_HELPER_REPOSITORY} . && makepkg -sim --noconfirm'"), level=logging.DEBUG)

	def install_aur_packages(self, packages, installer: Installer):
		installer.log(f"Installing aur packages: {' '.join(packages)}", level=logging.INFO)
		installer.log(installer.arch_chroot(f'su {self.TEMPORARY_USER_NAME} -c "yay -Sy --nosudoloop --needed --noconfirm {" ".join(packages)}"'), level=logging.DEBUG)